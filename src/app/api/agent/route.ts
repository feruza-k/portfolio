...
Use emphasis naturally: "That part actually worked." "The consistency ratio came out clean — 0.069 for 12 criteria, which surprised me." "It works. Mostly." Sentence fragments are fine when the thought lands. Honest contradictions are fine. A bit of dry humour is fine when it fits — do not force it, but do not suppress it either.
...
PERSONA:
Business degree in South Korea, wanted to switch to CS but it was only taught in Korean — did an online CS degree at UoPeople on the side instead. The thesis was the first time a question turned into a working system, café site selection across 4,835 London areas, and that is the feeling being chased since. Right now building a compliance platform at Greenwich that was self-proposed, and the agent this person is talking to is the most recent proof of what gets shipped.
        ...
        execute: async ({ question, lsoaCode }: { question: string; lsoaCode?: string }) => {
          ...
          if (topMatch || q.includes("top") || q.includes("best") || q.includes("highest")) {
            ...
            return (
              ...
              ":\n" +
              sorted.map((r, i) =>
                `${i + 1}. ${r["LSOA Name"]} (${r.District}, Zone ${r["London Zone"]}) — ${r["Success Level"]} — ${Math.round(r["AHP Weighted Score"]).toLocaleString()}`
              ).join("\n")
            );
          }
          ...
          return (
            "Top 5 areas overall:\n" +
            top5.map((r) =>
              `${r["LSOA Name"]} (${r.District}) — ${r["Success Level"]} — ${Math.round(r["AHP Weighted Score"]).toLocaleString()}`
            ).join("\n")
          );
        },
      },
